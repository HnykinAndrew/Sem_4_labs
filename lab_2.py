...
class CalcMesh:
    ...
    def __init__(self, nodes_coords, tetrs_points):
        ...
        self.len = np.power(np.power(self.nodes[0] - self.x_0, 2) + np.power(self.nodes[1] - self.y_0, 2), 1/2)
        self.alpha = []
        for i in range(0, len(self.nodes[0])):
        ...
        self.velocity[0] = -self.len * np.sin(self.alpha)
        self.velocity[1] = self.len * np.cos(self.alpha)
        self.velocity[2] = self.len * 0

    # Метод отвечает за выполнение для всей сетки шага по времени величиной tau
    def move(self):
        # По сути метод просто двигает все точки c их текущими скоростями
        self.nodes[0] = self.len * np.cos(self.alpha) + self.x_0
        self.nodes[1] = self.len * np.sin(self.alpha) + self.y_0
        self.velocity[0] = -self.len * np.sin(self.alpha)
        self.velocity[1] = self.len * np.cos(self.alpha)
...
try:
    ...
    gmsh.merge(os.path.join(path, 'lab_1_2.stl'))
except:
